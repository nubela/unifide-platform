...
Template.account_brand.events = {
    ...
    'click #get_facebook_pages': function(event) {
        ...
        Meteor.call("get_facebook_pages", Session.get("account_brand"), function(error, result) {
            page_loading(false);
            _FBPages.remove({});
            for (var i=0;i<result.length;i++) { _FBPages.insert({id: result[i].id, name: result[i].name, category: result[i].category}); }
...
Template.account_brand.facebook_account = function() {
    ...
    if (user_fb == undefined) { return no_account_added; }
    else { return Template["account_brand_facebook"](); }
}
...
Template.account_brand.facebook_add = function() {
    if (FBUsers.findOne() == undefined) { return true; }
    else { return false; }
}
...
Template.account_brand.twitter_add = function() {
    if (TWUsers.findOne() == undefined) { return true; }
    else { return false; }
}
...
Template.account_brand.foursquare_add = function() {
    if (FSQUsers.findOne() == undefined) { return true; }
    else { return false; }
}

Template.account_brand.brand_keywords = function() {
    return no_keywords_added;
}
...
function page_loading(bool) {
    if (bool) { $('body').css({"cursor": "progress"}); }
